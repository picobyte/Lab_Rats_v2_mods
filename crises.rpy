...
init 1 python:
    ...
    def in_research_with_other(): #A common requirement check, the PC is in the office (not nessesarily the lab), during work hours, with at least one other person.
        if world.mc.business.is_open_for_business(): #Only trigger if people are in the office.
            if world.mc.is_at_work(): #Check to see if the main character is at work
                if len(world.mc.business.r_div.people) > 0: #Check to see if there's at least one person in the research team team at work and that something is being researched.
                        return True
        return False
    ...
    def in_production_with_other():
        if world.mc.business.is_open_for_business(): #Only trigger if people are in the office.
            if world.mc.is_at_work(): #Check to see if the main character is at work
                if len(world.mc.business.p_div.people) > 0: #Check to see if there's at least one person in the research team team at work and that something is being researched.
                        return True
        return False
        
    def anyone_else_in_office(): #Returns true if there is anyone else at work with the world.mc.
        if world.mc.business.is_open_for_business:
            if world.mc.is_at_work():
                if world.mc.business.get_employee_count() > 0:
                    return True
        return False
    ...
    def mc_asleep(): #Returns true if the main character is at home and in bed.
        if world.time_of_day == 4: #It has to be after work, right when you've gone to bed.
            if world.mc.location == world["bedroom"]:
                return True
        return False
    ...
    def broken_AC_crisis_requirement():
        if world.mc.business.is_open_for_business(): #Only trigger if people are in the office.
            if world.mc.is_at_work(): #Check to see if the main character is at work
                if len(world.mc.business.p_div.people) > 0: #Check to see if there's at least one person in the production team at work.
                    return True
        return False
...
label broken_AC_crisis_label:
    ...
    $ renpy.show(world.mc.business.p_div.name,what=world.mc.business.p_div.background_image) #Just in case another crisis had interupted us being here.
    ...
    menu:
        ...
        "Tell everyonyone to strip down and keep working.":
            if len(world.mc.business.p_div.people) > 1: #We have more than one person, do a group strip scene.
                world.mc.name "I know it's uncomfortable in here right now, but we're just going to have to make due."
                world.mc.name "If anyone feels the need to take something off to get comfortable, I'm lifting the dress code until the air conditioning is fixed."
                #We're going to use the most slutty girl of the group lead the pack. She'll be the one we pay attention to.
                python:
                    the_person = None
                    for girl in world.mc.business.p_div.people:
                        if not the_person:
                            the_person = girl
                        else:
                ...
                $ the_person = world.mc.business.p_div.people[0] #Get the one person, the crisis requires we have at least 1 person in here so this should always be true.
                $ the_person.draw_person()
                world.mc.name "[the_person.name], I know it's uncomfortable in here right now, but we're going to have to make due."
                world.mc.name "If you feel like it would help to take something off, I'm lifting the dress code until the air condition is fixed."
            ...
            if len(world.mc.business.p_div.people) > 1:
                if removed_something:
...
init 1 python:
    def special_training_requirement():
        if world.mc.business.get_employee_count() > 0: #As long as you have at least one person working for you.
            if world.mc.business.is_open_for_business(): # Only triggers during the world.day when people are working.
                return True
...
label special_training_crisis_label():
    if not world.mc.business.get_employee_count() > 0:
        return #We must have had someone quit or be fired, so we no longer can get a random person.
        
    $ the_person = renpy.random.choice(world.mc.business.get_employee_list())
    show screen person_info_ui(the_person)
    hide screen business_ui
    "You get an email from [the_person.name]."
    the_person.name "[world.mc.name], I've just gotten word about a training seminar going on right now a few blocks away. I would love to take a trip over and see if there is anything I could learn."
    the_person.name "There's a sign up fee of $500. If you can cover that, I'll head over right away."
    menu:
        "Send [the_person.name] to the Seminar. -$500" if world.mc.business.funds >= 500:
            $ world.mc.business.funds += -500
            "You type up a response."
            world.mc.name "That sounds like a great idea. I'll call and sort out the fee, you start heading over."
        ...
        "Tell her to stay at work.":
            "You type up a response."
            world.mc.name "I'm sorry [the_person.name], but there aren't any extra funds in the budget right now."
...
init 1 python:
    def lab_accident_requirement():
        if in_research_with_other():
            if world.mc.business.active_research_design and "value" in world.mc.business.active_research_design:
                return True
        return False
...
label lab_accident_crisis_label():
    ...
    if not (world.mc.business.active_research_design and world.mc.business.is_reasearching_drug()):
        return
...
init 1 python:
    def production_accident_requirement():
        if in_production_with_other():
            if world.mc.business.serum_production_target: #Check to see if there's at least one person in the production department and that we're serum right now.
                return True
        return False
...
label production_accident_crisis_label():
    ## Some quick checks to make sure the crisis is still valid (for example, a serum being finished before this event can trigger)
    if not world.mc.business.serum_production_target:
        return
...
label water_spill_crisis_label():
    ...
    world.mc.name "Hey [the_person.name], how can I help you?"
    ...
    if not thinks_appropriate:
        the_person.name "I'm so sorry about this [world.mc.name], I just... I just need to go and dry this off!"
        $ renpy.scene("Active")
        ...
        world.mc.name "Of course, back to your taxes then, right?"
        ...
        if not thinks_appropriate:
            the_person.name "I'm so sorry about this [world.mc.name]. Let me just take this off, you keep talking."
            ...
            world.mc.name "Right, your taxes..."
            ...
            the_person.name "I'm so sorry about this [world.mc.name], should I go dry this off first?"
            menu:
                "Dry it off now.":
                    world.mc.name "You go dry it off, I'll wait here for you."
                ...
                "Leave it alone.":
                    world.mc.name "I'd like to get back to work as quickly as possible, just leave it for now and you can dry it off later."
                ...
                "Take it off.":
                    world.mc.name "I'm really quite busy right now, just take it off now and you can dry it off later."
...
init 1 python:
    def home_fuck_crisis_requirement():
        if mc_asleep():
            if world.mc.business.get_max_employee_slut()>=15: #We need them to start with at least sluttiness 15 so a bonus and some foreplay will get you up to sex range.
                return True
        return False
...
label home_fuck_crisis_label():
    ...
    python:
        for person in world.mc.business.get_employee_list():
    ...
    world.mc.name "[the_person.name], what are you doing here? It's the middle of the night."
    "[the_person.name] takes a step towards you, running a hand down your chest."
    the_person.name "Oh [world.mc.name], I just had the worst night and I need you to help me!"
    ...
    menu:
        "Fuck her.":
            ...
            if the_person.arousal > 100:
                ...
                the_person.name "Mmm, that was just what I needed [world.mc.name]. Ah..."
                "You and [the_person.name] lounge around for a few minutes until she has completely recovered."
                the_person.name "I had a great time [world.mc.name], but I should be getting home. Could you call me a cab?"
        ...
        "Ask her to leave.":
            world.mc.name "[the_person.name], you're drunk and not thinking straight. I'll call you a cab to get you home, in the morning this will all seem like a bad idea."
            $ the_person.draw_person(emotion = "sad")
            ...
            world.mc.name "Trust me, it's for the best."
            $ the_person.change_obedience(1)
            show screen float_up_screen(["+1 Obedience"],["float_text_grey"])
...
label quitting_crisis_label(the_person): #The person tries to quit, you have a chance to keep her around for a hefty raise (Or by fucking her, if her sluttiness is high enough).
    if not world.mc.business.is_employee(the_person):
    ...
    menu:
        "Offer a 10%% raise. (+$[raise_amount]/world.day)":
            world.mc.name "I'm very sorry to hear that [the_person.name], I understand that your job can be difficult at times."
            "You pull out [the_person.name]'s employee records and look them over."
            world.mc.name "In fact, the fact that you're being paid as little as you are is criminal. If I promised you a 10%% raise, effective immediately, would you consider staying on board?"
            ...
            if the_person.get_job_happiness_score() >= 0: #The raise has made them happy with the job.
                ...
                world.mc.name "Excellent, I'll have it on the books by the end of the world.day. Thank you for all your hard work [the_person.name]."
        ...
        "Make her cum to convince her to stay." if the_person.effective_sluttiness() > 60:
            ...
            world.mc.name "[the_person.name], you've always been a good employee of mine."
            ...
            world.mc.name "Let me show you the perks of working around here, if you still want to quit after then I won't stop you."
            ...
            if the_person.arousal > 100: #If you made them cum, they'll stay on for a little while.
                ...
                world.mc.name "Well [the_person.name], are you still thinking of leaving?"
                ...
                world.mc.name "It was my pleasure."
                ...
                the_person.name "I'm sorry [world.mc.name], but I haven't changed my mind. I'll clear out my desk and be gone by the end of the world.day."
                "[the_person.name] takes a moment to put herself back together, then steps out of your office."
                $ world.mc.business.remove_employee(the_person)
            $ the_person.reset_arousal()
        ...
        "Let them go.":
            world.mc.name "I'm sorry to hear that [world.mc.name], but if that's the way you feel then it's probably for the best."
...
label serum_creation_crisis_label(the_serum): # Called every time a new serum is created, test it on a R&D member.
    ...
    if rd_staff is not None:
        ...
        if world.mc.location == the_place: # The MC is in the lab, just physically get them.
            ...
            menu:
                "Oversee final testing of the new serum.":
                    world.mc.name "I think I would. Show me what you've done."
                    #Fall through to the next section.
                    
                "Leave it to your R&D crew.":
                    world.mc.name "Thank you for letting me know [rd_staff.name], but I'm currently busy. I'm sure you're capable of running whatever tests seem necessary."
            ...
            $ renpy.say(rd_staff.name, "%s, I've had a breakthrough! The first test dose of serum \"%s\" is coming out right now!" % (world.mc.name, the_serum["name"]))
        ...
        menu:
            "Give the serum back for final testing.":
                ...
                if rd_staff.sluttiness < 10:
                    world.mc.name "I'm sure you will. Keep up the good work."
            ...
            "Test the serum on [rd_staff.name].":
                world.mc.name "How confident in your work are you [rd_staff.name]? We could fast track this over to production if you're willing to take this test dose right now."
                if rd_staff.obedience < 80:
                    ...
                    world.mc.name "Fine, just make sure you get it done."
                    ...
                    menu:
                        "Give [rd_staff.name] a %%10 raise. (+$[raise_amount]/world.day)":
                            show screen float_up_screen(["+$[raise_amount]/world.day Salary"],["float_text_green"])
                            world.mc.name "Alright, you've got yourself a deal. I'll have the books updated by the end of the world.day."
                        ...
                        "Refuse.":
                            world.mc.name "I'm sorry but that just isn't in the budget right now."
        ...
        world.mc.name "Thank you for the help [rd_staff.name]."
